...
TEST(Liveness, Loop3) {
    ...
    EXPECT_EQ(lifetimes.at(zero), bjac::Lifetime({Segment(1, 4), Segment(9, 9)}));
    EXPECT_EQ(lifetimes.at(one), bjac::Lifetime{Segment(2, 10)});
    ...
    EXPECT_EQ(lifetimes.at(next_acc), bjac::Lifetime({Segment(6, 8), Segment(10, 10)}));
    EXPECT_EQ(lifetimes.at(next_counter), bjac::Lifetime({Segment(7, 8), Segment(10, 10)}));
}
